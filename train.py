...
def train_model(model, optimizer, data_o, train_loader, val_loader, test_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        if not args.fastmode:
            roc_val, prc_val, f1_val, acc_val, recall_val, loss_val = test(model, val_loader, data_o, args)
            if roc_val > max_auc:
                model_max = copy.deepcopy(model)
                max_auc = roc_val
    ...
    # Testing
    auroc_test, prc_test, f1_test, acc_test, recall_test, loss_test = test(model_max, test_loader, data_o, args)
    print('loss_test: {:.4f}'.format(loss_test.item()), 'auroc_test: {:.4f}'.format(auroc_test),
          ...
          'acc_test: {:.4f}'.format(acc_test), 'recall_test: {:.4f}'.format(recall_test))

    with open(args.out_file, 'a') as f:
        f.write('{0}\t{1}\t{2}\t{9}\t{3:.4f}\t{4:.4f}\t{5:.4f}\t{6:.4f}\t{7:.4f}\t{8:.4f}\n'.format(
            args.in_file[5:8], args.seed, args.aggregator, loss_test.item(), auroc_test, prc_test, f1_test, acc_test, recall_test,
            args.feature_type))


def test(model, loader, data_o, args):
    m = torch.nn.Sigmoid()
    loss_fct = torch.nn.BCELoss()
    b_xent = nn.BCEWithLogitsLoss()
    model.eval()
    ...
    return roc_auc_score(y_label, y_pred), average_precision_score(y_label, y_pred), f1_score(y_label, outputs), \
           accuracy_score(y_label, outputs), recall_score(y_label, outputs), loss
