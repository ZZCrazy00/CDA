...
def load_data(args, val_ratio=0.1, test_ratio=0.2):
    ...
    positive0 = np.loadtxt(args.in_file, dtype=np.int64)

    fr_tor = open("data/CDA_torsion_1-1.edgelist", 'r')
    dict_tor = {}
    for line in fr_tor:
        arrline = line.strip().split("\t")
        dict_tor[arrline[0] + "-" + arrline[1]] = float(arrline[2])

    G = nx.Graph()
    G.add_edges_from(positive0)
    unique_entity = len(G.nodes)

    link_size = int(positive0.shape[0] * args.network_ratio)
    np.random.seed(args.seed)
    np.random.shuffle(positive0)
    positive = positive0[:link_size]
    ...
    for a in range(edge_index_o.shape[1]):
        if edge_index_o[0, a] == edge_index_o[1, a]:
            ...
            try:
                tor = dict_tor[str(row) + "-" + str(col)]
            except:
                tor = dict_tor[str(col) + "-" + str(row)]
            edge_cuva.append(tor)
    ...
    # gene_feature = pd.read_csv("data/gene_seq_sim.csv", index_col=0)
    # drug_feature = pd.read_csv("data/drug_str_sim.csv", index_col=0)
    # padding = pd.DataFrame(
    #     np.zeros((drug_feature.shape[0], 53)),
    #     index=drug_feature.index
    # )
    # drug_feature = pd.concat([drug_feature, padding], axis=1)
    #
    # features = np.vstack([gene_feature.values, drug_feature.values])
    ...
    args.dimensions = features.shape[1]

    edge_cuva_all = torch.tensor(edge_cuva, dtype=torch.float)
    x_o = torch.tensor(features, dtype=torch.float)
